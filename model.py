...
from keras.layers import Input, concatenate, Lambda, LeakyReLU, MaxPooling2D
...
class Multimodel(object):
    ...
    def encoder_maker(self, modality):
        ...
        skip = concatenate([act2, conv], mode='concat', concat_axis=1, name='enc_' + modality + '_skip1')
        ...
        skip = concatenate([act1, conv], mode='concat', concat_axis=1, name='enc_' + modality + '_skip2')
    ...
    def decoder_maker(self, modality):
        ...
        skip = concatenate([inp, conv], mode='concat', concat_axis=1, name='dec_' + modality + '_skip1')
        ...
        skip = concatenate([skip, conv], mode='concat', concat_axis=1, name='dec_' + modality + '_skip2')
    ...
    def get_embedding_distance_outputs(self, embeddings):
        ...
        concat_emb = concatenate(all_emb_flattened, mode='concat', concat_axis=1, name='em_concat')
    ...
    def hemis(self, ind_emb):
        if len(self.input_modalities) == 1:
            ...
            combined_emb1 = concatenate(ind_emb, mode='ave', name='combined_em_ave',
                                  output_shape=(self.latent_dim / 2, None, None))
            combined_emb2 = concatenate(ind_emb, mode=var, name='combined_em_var',
                                  output_shape=(self.latent_dim / 2, None, None))

        combined_emb = concatenate([combined_emb1, combined_emb2], mode='concat', concat_axis=1, name='combined_em',
                             output_shape=(self.latent_dim, None, None))

        new_ind_emb = []
        for i, emb in enumerate(ind_emb):
            new_ind_emb.append(concatenate([emb, zeros_for_var(emb)], mode='concat', concat_axis=1, name='emb_' + str(i),
                                     output_shape=(self.latent_dim, None, None)))
        ind_emb = new_ind_emb
    ...
    def build(self):
        ...
        if self.spatial_transformer:
            ...
            for mod in ind_emb[1:]:
                aligned_mod = concatenate([tpn([mod1, mod]), mod], mode=STMerge, output_shape=input_shape)
        ...
        if self.common_merge == 'hemis':
            ...
            weighted_rep = concatenate(ind_emb, mode=cm, name='combined_em') if len(self.inputs) > 1 else ind_emb[0]
    ...
    def get_embeddings(self, modalities):
        ...
        if self.common_merge == 'hemis':
            combined_emb1 = concatenate(org_ind_emb, mode='ave', name='combined_em_ave',
                                  output_shape=(self.latent_dim / 2, None, None))
            combined_emb2 = concatenate(org_ind_emb, mode=var, name='combined_em_var',
                                  output_shape=(self.latent_dim / 2, None, None))
            ...
            for i, emb in enumerate(org_ind_emb):
                new_ind_emb.append(concatenate([emb, zeros_for_var(emb)], mode='concat', concat_axis=1, name='pemb_' + str(i),
                                         output_shape=(self.latent_dim, None, None)))
            ind_emb = new_ind_emb
            ...
            if len(ind_emb) > 1:
                fused_emb = concatenate(ind_emb, mode=self.common_merge, name='fused_em')
            else:
                fused_emb = ind_emb[0]
...
def tpn_maker(input_shape):
    ...
    stacked = concatenate([target_input, input], mode='concat')
